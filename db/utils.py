...
from .models import db, Device, DeviceState, DeviceSetting, DeviceConfig, Payment, AckMessage, DisplayInfo, Sale, Collection
...
def get_or_create_device(device_id):
    ...
    if not device:
        try:
            device = Device(id=device_id, last_seen=datetime.utcnow(), monobank_api_key=None)
            db.session.add(device)
            db.session.commit()
        except IntegrityError:
            db.session.rollback()
            device = Device.query.filter_by(id=device_id).first()  # Повторный запрос после rollback
...
def save_device_state(device_id, state_data):
    ...
    device = get_or_create_device(device_id)
    if not device:
        current_app.logger.error(f"Cannot save state for non-existent device {device_id}")
        return None
...
def get_latest_device_state(device_id):
    ...
    try:
        return DeviceState.query.filter_by(device_id=device_id).order_by(DeviceState.created.desc()).first()
    except Exception as e:
...
def save_device_settings(device_id, settings_data):
    ...
    device = get_or_create_device(device_id)
    if not device:
        current_app.logger.error(f"Cannot save settings for non-existent device {device_id}")
        return None
...
def get_latest_device_settings(device_id):
    ...
    try:
        return DeviceSetting.query.filter_by(device_id=device_id).order_by(DeviceSetting.created.desc()).first()
    except Exception as e:
...
def save_device_config(device_id, config_data):
    ...
    device = get_or_create_device(device_id)
    if not device:
        current_app.logger.error(f"Cannot save config for non-existent device {device_id}")
        return None
...
def get_latest_device_config(device_id):
    ...
    try:
        return DeviceConfig.query.filter_by(device_id=device_id).order_by(DeviceConfig.created.desc()).first()
    except Exception as e:
...
def get_latest_ack_message(device_id, message_type):
    ...
    try:
        return AckMessage.query.filter_by(
            device_id=device_id, 
            message_type=message_type
        ).order_by(AckMessage.created.desc()).first()
    except Exception as e:
...
@safe_db_operation
def save_payment(device_id, payment_type, amount, order_id=None, status='pending', monobank_invoice_id=None, confirmed_at=None):
...
def get_latest_display_info(device_id):
    ...
    try:
        return DisplayInfo.query.filter_by(device_id=device_id).order_by(DisplayInfo.created.desc()).first()
    except Exception as e:
...
def save_sale(device_id, sale_data):
    ...
    sale = Sale(
        ...
        created=datetime.strptime(sale_data.get('created', ''), '%Y-%m-%dT%H:%M:%S') if 'created' in sale_data else datetime.utcnow(),
...
def get_sale(device_id, external_id):
    ...
    try:
        return Sale.query.filter_by(device_id=device_id, external_id=external_id).first()
    except Exception as e:
...
def get_device_sales(device_id, limit=100):
    ...
    try:
        return Sale.query.filter_by(device_id=device_id).order_by(Sale.created.desc()).limit(limit).all()
    except Exception as e:
...
def save_collection(device_id, collection_data):
    ...
    collection = Collection(
        ...
        created=datetime.strptime(collection_data.get('created', ''), '%Y-%m-%dT%H:%M:%S') if 'created' in collection_data else datetime.utcnow(),
...
def get_collection(device_id, external_id):
    ...
    try:
        return Collection.query.filter_by(device_id=device_id, external_id=external_id).first()
    except Exception as e:
...
def get_device_collections(device_id, limit=100):
    ...
    try:
        return Collection.query.filter_by(device_id=device_id).order_by(Collection.created.desc()).limit(limit).all()
    except Exception as e:
